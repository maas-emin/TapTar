...
function AffiliationFile() {
  ...
  if (files.type === 'photo') {
    media = (
      <img
        src={`${process.env.REACT_APP_TEPTAR_HOST}/storage/${files.path_to_file}`}
        alt="#"
      />
    );
  }
  if (files.type === 'video') {
    media = (
      <ReactPlayer
        width="100%"
        height="100%"
        url={`${process.env.REACT_APP_TEPTAR_HOST}/storage/${files.path_to_file}`}
        controls
      />
    );
  }
  if (files.type === 'audio') {
    media = <AudioPlayer path={files.path_to_file} />;
  ...
  if (files.type === 'document') {
    media = <Document fill="#fff" path={files.path_to_file} />;
