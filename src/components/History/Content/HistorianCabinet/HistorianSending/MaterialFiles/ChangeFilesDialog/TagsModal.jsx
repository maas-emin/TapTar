...
function TagsModal(props) {
  ...
  if (props.tagsModal.title === 'Теги (тип/принадлежность):') {
    titleTag = props.information;
    itemTag = types;
    addedTitle = 'Теги (тип/принадлежность):';
  }
  if (props.tagsModal.title === 'Теги (период/век):') {
    titleTag = props.century;
    itemTag = centuries;
  ...
  const handleAddedCredibilityClick = (tag, checkTag) => {
    ...
    if (props.tagsModal.title === 'Теги (тип/принадлежность):') {
      const newInformation = [...props.information, tag];
      return props.setInformation(newInformation);
    }
    if (props.tagsModal.title === 'Теги (период/век):') {
      const newCentury = [...props.century, tag];
      return props.setCentury(newCentury);
    }
  };

  const handleRemoveCredibilityClick = (id) => {
    if (props.tagsModal.title === 'Теги (тип/принадлежность):') {
      const newInformation = props.information.filter((item) => item.id !== id);
      return props.setInformation(newInformation);
    }
    if (props.tagsModal.title === 'Теги (период/век):') {
      const newCentury = props.century.filter((item) => item.id !== id);
      return props.setCentury(newCentury);
    }
  };
